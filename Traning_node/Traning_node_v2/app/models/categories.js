...
module.exports = {
    ...
    countItems: (params, options = null)=>{
      return  categoriesService.count(params.objWhere)
    },
    changeStatus: (currentStatus,id, options = null)=>{
        ...
        if (options.task == 'changeStatus-multi') {
           ...
           return categoriesService.updateMany({_id:{$in:id}}, data)
        } 
        if (options.task == 'changeStatus-one') {
            data.status = status
            return categoriesService.updateOne({_id: id}, data )
        }
      },
      changeOrdering: async (cids,oderings, options = null)=>{
        ...
        if(Array.isArray(cids)){
            for (let index = 0; index < cids.length; index++) {
                ...
                await categoriesService.updateOne({_id: cids[index]}, data)
            }
            Promise.resolve("Success")
        }else{
         
         return categoriesService.updateOne({_id: cids}, data)
        }
      },
      deleteItems: (id, options=null )=>{
        if (options.task == 'delete-multi') {
           return categoriesService.deleteMany({_id:{$in:id}})
        } 
        if (options.task == 'delete-one') {
           return  categoriesService.deleteOne({_id: id})
        }
      ...
      saveItems: (item, options = null) =>{
        if (options.task == 'add') {
          ...
          return  new categoriesService(item).save()
        } 
        if (options.task == 'edit') {
        return  categoriesService.updateOne({_id: item.id},{
            name: item.name,
