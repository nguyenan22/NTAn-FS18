...
const CategoriesModels = require(__path_models + colName)
...
let pageTitle = `Category Manager`
...
let statusFillters = await utilsHelpers.createStatusFilter(params.currentStatus, colName)
...
router.get('/change-status/:status/:id', async function(req, res, next) {
  ...
  await CategoriesModels.changeStatus(currentStatus,id, {task:"changeStatus-one"}).then((result)=>{
...
router.get('/delete/:id', async function(req, res, next) {
  let id = paramsHelpers.getParam(req.params,'id', '')
  await CategoriesModels.deleteItems(id, {task: 'delete-one'}).then((result)=>{
...
router.post('/change-ordering/', async function(req, res, next) {
  ...
  CategoriesModels.changeOrdering(cids, ordering).then((result)=>{
...
router.post('/change-status/:status', async function(req, res, next) {
  let currentStatus = paramsHelpers.getParam(req.params,'status', 'active')
  await CategoriesModels.changeStatus(currentStatus,req.body.cid,{task:'changeStatus-multi'}).then((data)=>{
...
router.post('/delete/', async function(req, res, next) {
  await CategoriesModels.deleteItems(req.body.cid,{task: 'delete-multi'} ).then((data)=>{
