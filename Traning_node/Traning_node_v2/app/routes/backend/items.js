...
await ItemModels.countItems(params).then((data)=>{
  params.pagination.totalItems = data
 })
 
 ItemModels.listItems(params)
    .then((items)=>{
...
router.get('/change-status/:status/:id', async function(req, res, next) {
  ...
  await ItemModels.changeStatus(currentStatus,id, {task:"changeStatus-one"}).then((result)=>{
...
router.get('/delete/:id', async function(req, res, next) {
  let id = paramsHelpers.getParam(req.params,'id', '')
  await ItemModels.deleteItems(id, {task: 'delete-one'}).then((result)=>{
...
router.post('/change-ordering/', async function(req, res, next) {
  ...
  ItemModels.changeOrdering(cids, ordering).then((result)=>{
...
router.post('/change-status/:status', async function(req, res, next) {
  let currentStatus = paramsHelpers.getParam(req.params,'status', 'active')
  await ItemModels.changeStatus(currentStatus,req.body.cid,{task:'changeStatus-multi'}).then((data)=>{
...
router.post('/delete/', async function(req, res, next) {
  await ItemModels.deleteItems(req.body.cid,{task: 'delete-multi'} ).then((data)=>{
