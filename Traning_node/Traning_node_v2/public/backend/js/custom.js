...
$(document).ready(function () {
    ...
    $('select[name="filter-group"]').change(function (value) {
        ...
        const linkRedirect = '/'+ path[1] + '/' + path[2] + '/filter-group/' + $(this).val()
        // gửi link-redirect
    ...
    // show ảnh khi có file ảnh dc add vào
    function readURL(input,style ='') {
        if (input.files && input.files[0]) {
            var reader = new FileReader();
    
            reader.onload = function (e) {
                if ($('#include-image').length){
                    $('#include-image').attr('src', e.target.result);
                } else {
                    var img = $('<img />', { 
                        id: 'include-image',
                        src: '',
    ...
    $("#ImageAvatarUsers").change(function(){ 
        readURL(this),'height: 50px;width: 50px; border-radius:50%'})
...
$('input[name="name"]').keyup(function(){
    $('#change-slug').val(ChangeToSlug($(this).val()));
})
...
// change category
$('select[name="change-categoryId"]').change(function (value) {
    const path = window.location.pathname.split('/')
    if ($(this).val()=='allvalue') {
        alert('Vui lòng chọn một Category')
        window.location.pathname = '/'+ path[1] + '/' + path[2]
    }else {
        const linkRedirect = '/'+ path[1] + '/' + path[2] + '/change-category/' + $(this).val() +'/'+ $(this).attr('id')
