...
const CategoriesServices = require(__path_services + colName)
...
await CategoriesServices.countItems(params).then((data)=>{
  params.pagination.totalItems = data
 })
 
 CategoriesServices.listItems(params)
    .then((items)=>{
...
router.post('/delete/(:id)?', async function(req, res, next) {
  ...
  if (req.params.id==='multi'){
    ...
    await CategoriesServices.deleteItems(arrId,{task: 'delete-multi'} ).then((data)=>{
    ...
    let id = req.body.id
    await CategoriesServices.deleteItems(id, {task: 'delete-one'}).then((result)=>{
...
router.post('/change-ordering/', async function(req, res, next) {
  ...
  CategoriesServices.changeOrdering(id, ordering).then((result)=>{
...
router.post('/change-status/(:status)?', async function(req, res, next) {
  ...
  console.log(req.params)
    if (status=='multi'){
      await CategoriesServices.changeStatus(req.body.status,req.body.id.split(','),{task:'changeStatus-multi'}).then((data)=>{
      ...
      let {status, id}=req.body
       await CategoriesServices.changeStatus(status,id, {task:"changeStatus-one"}).then((result)=>{
      //     // req.flash('warning',notifyConfig.CHANGE_STATUS_SUCCESS ,linkIndex)
          res.send({success: true})
       })
    }
...
router.get('/form(/:id)?', async function(req, res, next) {
  ...
  if (id === '') { //add
    ...
    await CategoriesServices.getItems(id).then((item)=>{
...
async function(req, res, next) {
  ...
  if (!errors.isEmpty()) {
    ...
    CategoriesServices.saveItems(item,{task: taskCurrent}).then((result)=>{
